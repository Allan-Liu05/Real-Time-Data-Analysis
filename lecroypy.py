'''
Original Author: SengerM, original repository - https://github.com/SengerM/TeledyneLeCroyPy
Modified By: Allan Liu

Modification can by found on lines 602-608. 
'''
import time
import numpy as np
import pyvisa
...
class LeCroyWaveRunner:
	...
	def get_waveform(self, n_channel:int)->dict:
		...
		# original forces a time axis start of 0, but oscilloscope raw data may not necessarily start at 0 due to user manipulation
		# such as horizontal offsetting so I modified this method to account for that
		time_array = np.arange(
			start = parsed_wavedesc_block['HORIZ_OFFSET'],
			stop = parsed_wavedesc_block['HORIZ_OFFSET']+parsed_wavedesc_block['HORIZ_INTERVAL']*(n_samples_per_trigger), 
